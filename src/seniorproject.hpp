...
inline seniorproject::seniorproject()
    {
    //std::cout << "Constructing a seniorproject" << std::endl;
    clear();
    }

inline seniorproject::~seniorproject()
    {
    //std::cout << "Destroying a seniorproject" << std::endl;
    }

inline void seniorproject::clear()
    {
    _data_file_name = "";
    _tree_file_name = "";
    _data           = nullptr;
...
inline void seniorproject::processCommandLineOptions(int argc, const char * argv[])
    {
    ...
    try
        {
        const boost::program_options::parsed_options & parsed = boost::program_options::parse_config_file< char >("seniorproject.conf", desc, false);
        boost::program_options::store(parsed, vm);
        }
    catch(boost::program_options::reading_file & x)
        {
        std::cout << "Note: configuration file (seniorproject.conf) not found" << std::endl;
        }
    ...
    for (auto & freq : _state_frequencies)
        {
        if (freq <= 0.0)
            throw Xseniorproject("all statefreq entries must be positive real numbers");
        freq /= sum_freqs;
        }
    ...
    for (auto & xchg : _exchangeabilities)
        {
        if (xchg <= 0.0)
            throw Xseniorproject("all rmatrix entries must be positive real numbers");
        xchg /= sum_xchg;
        }

    // Be sure gamma shape parameter is positive
    if (_gamma_shape <= 0.0)
        throw Xseniorproject("gamma shape must be a positive real number");

    // Be sure number of gamma rate categories is greater than or equal to 1
    if (_num_categ < 1)
        throw Xseniorproject("ncateg must be a positive integer greater than 0");

    }

inline void seniorproject::sample(unsigned iteration, Chain & chain)
    {
...
inline void seniorproject::run()
    ...
    try
        ...
        {
        std::cerr << "seniorproject encountered a problem:\n  " << x.what() << std::endl;
        }
