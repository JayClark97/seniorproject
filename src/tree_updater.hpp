...
inline void TreeUpdater::proposeNewState()
    ...
    double num_edges = (double)(_tree_manipulator->getTree()->numNodes() - 1);
    double TL = _tree_manipulator->calcTreeLength();
    double Pac = (_orig_edgelen_top + _orig_edgelen_middle + _orig_edgelen_bottom)/TL;
    _log_hastings_ratio = 3.0*log(m) - num_edges*log(Pac*m + 1.0 - Pac);
